...
with col_name:
    child_name = st.text_input("🎨 幼兒姓名 / 暱稱", placeholder="例如：芽芽")
with col_age:
    child_age = st.text_input("🎈 幼兒年齡", placeholder="例如：6歲3個月")

teacher_notes = st.text_area("✍️ 老師的補充觀察 (選填)", 
                            placeholder="描述孩子的口語反應、遇到挑戰時的反應等，能讓 AI 評估更貼近真實。")
...
if st.button("✨ 生成專業觀察報告"):
    if uploaded_file and child_age and child_name:
        with st.spinner(f"正在分析 {child_name} 的發展軌跡..."):
            try:
                img = Image.open(uploaded_file)
                
                # 核心分析指令：整合「三步溝通法」與「反過度美化」邏輯
                prompt = f"""
                你是一位專業的幼教發展專家與親職溝通顧問。請觀察照片並結合以下資訊：
                - 幼兒姓名：{child_name}
                - 幼兒年齡：{child_age}
                - 老師筆記：{teacher_notes if teacher_notes else "無"}

                請嚴格依照以下結構輸出報告，語氣需溫暖、專業且客觀：
                ...
                3. **誠實觀測**：若能力低於實齡，請描述為「正致力於基礎經驗的積累」，嚴禁給予無根據的誇獎（如孩子分心卻誇專注），請改為描述其對環境的探索。
