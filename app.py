...
with st.expander("💡 第一次使用？點我查看【自訂模板標籤】與【本機自動歸檔教學】🌸"):
    ...
    ### 📋 一、 學校既有 Word 表格標籤設定
    請在您電腦裡既有的 Word 空白紀錄表中，將需要讓系統代筆的地方，打上對應的「魔法標籤」（請連同大括號一起複製貼上喔！）：
    * 🖍️ **基本資訊**：`{{幼兒姓名}}`、`{{幼兒年齡}}`
    * 📝 **專業分析**：`{{活動紀錄}}`、`{{領域分析}}`、`{{能力評估}}`
    * 🌱 **支持策略**：`{{智慧鷹架}}`
    * 💌 **家長視角**：`{{親師溝通}}`
    * 🖼️ **動態相片牆**：請在表格的空白格子內填入 `{{照片歷程}}`。
    
    > 💡 **老師的彈性自訂小技巧**：如果學校的表格很精簡，只需要「姓名」和「親師溝通」，您在 Word 裡就**只要貼這兩個標籤就好**！其他沒貼的標籤系統會自動忽略，完全不會寫入 Word，但完整的分析依然會在網頁畫面上秀出來給您參考，請放心依據需求設計。
    
    ---
    
    ### 📂 二、 獨家大絕招：網頁按下一鍵，自動飛進電腦指定資料夾！
    不想每次下載都彈出視窗、手動選儲存位置嗎？不需要安裝任何雲端軟體，直接讓瀏覽器幫您自動歸檔：
    
    1. **建資料夾**：先在您電腦桌面新增一個專屬資料夾（例如命名為：`2026學期幼兒觀察紀錄`）。
    2. **打開設定**：打開 Chrome 瀏覽器，點擊右上角 **「三個點（選單）」** ➜ 選擇 **「設定」**。
    3. **變更路徑**：點選左側選單的 **「下載」**：
       * 將「位置」更改為您剛剛在桌面建好的那個資料夾。
       * 將「下載每個檔案前先詢問儲存位置」**關閉（Turn Off）**。
    
    ✨ **神奇效果**：設定完成後，未來只要在系統點擊【💾 匯出 Word 報告】，檔案就會像變魔法一樣，完全不著痕跡地直接進到您桌面的資料夾裡，行政效率瞬間飆升！
    """)
