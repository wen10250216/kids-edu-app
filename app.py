...
with st.expander("💡 第一次使用自訂模板？點我查看【專屬標籤設定教學】🌸"):
    ...
    **Q：我學校的表格很精簡，只需要「姓名」和「親師溝通」怎麼辦？**
    * **A**：您**只需要在 Word 裡貼上 `{{幼兒姓名}}` 與 `{{親師溝通}}` 這兩個標籤就好囉！**
    * 其他不需要的標籤直接留空（不用貼）。系統非常聰明，沒貼的標籤會自動忽略、不會寫入 Word 檔中，但**完整的專業分析依然會在網頁畫面上秀出來給您參考**，完全不會影響系統運作，請放心依據學校需求彈性設計！
    """)
